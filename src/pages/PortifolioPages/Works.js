...
const Works = () => {
  ...
  function getUsers() {
    axios
      .get("http://localhost:80/myPortifolioApi/books/save")
      ...
      <div className="pt-32 px-4 md:mx-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 justify-center gap-3 mt-10 pb-20 md:pb-0">
          {users.length > 0 ? (
            users.map((user, index) => (
              <div key={user.id}>
                <ul className="bg-gradient-to-r from-pink-400 to-violet-700 w-full shadow-lg rounded-md py-6 px-4 md:px-8">
                  <li>
                    <h2 className="font-medium pb-4 text-white">
                      {user.project_title}
                    </h2>
                  </li>
                  <li>
                  ...
                  <li>
                    <div className="">
                            ...
                            <div className="">
                              ...
                              >
                                <button
                                  type="button"
                                  name="link_button"
                                  className="px-4 py-2 w-32 text-sm text-white md:text-slate-500 font-semibold rounded-md border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2"
                                >
                                  {" "}
                                  View{" "}
                                </button>
                      ...
                      <div
                        ref={arrowRef}
                        className="fixed bottom-16 right-20 cursor-pointer text-xl text-white bg-violet-600 rounded-full p-2 shadow"
                        onClick={handleClick}
                      >
                        <FaArrowUp />
                      </div>
            ...
            <ul>
              <li colSpan="6" className="py-2 px-4 text-center">
                No such book found.
              </li>
            </ul>
          )}
